...
const app = spawn(
  exe,
  ['.', '--force-device-scale-factor=1', `--remote-debugging-port=${PORT}`, `--user-data-dir=${probeUserDataDir}`],
  ...
  if (chromiumExe) {
    ...
    try {
      ...
      await guestPage.waitForSelector('#guest-room-container', { timeout: 15000 });
      {
        ...
        const geo = await page.evaluate(() => { const r = document.querySelector('.upper-canvas').getBoundingClientRect(); return { dpr: window.devicePixelRatio, sx: window.screenX, sy: window.screenY, chromeY: window.outerHeight - window.innerHeight, chromeX: (window.outerWidth - window.innerWidth) / 2, rx: r.x, ry: r.y }; });
        console.log('GEO', JSON.stringify(geo));
        const real = (ax, ay, bx, by) => { const f = (v, o) => Math.round(v * geo.dpr); const X = (cx) => Math.round((geo.sx + geo.chromeX + geo.rx + cx) * geo.dpr); const Y = (cy) => Math.round((geo.sy + geo.chromeY + geo.ry + cy) * geo.dpr); execFileSync('powershell', ['-NoProfile', '-File', 'C:/Users/alxch/AppData/Local/Temp/claude/c--desenv-utils-OneToOneSupport/30c3ebb0-718b-46e0-8de8-e1d49a57e5ee/scratchpad/drag.ps1', String(X(ax)), String(Y(ay)), String(X(bx)), String(Y(by))]); };
        for (const tool of ['pencil', 'rectangle', 'ellipse']) {
          await page.click('#tool-' + tool);
          real(150, 120, 330, 200);
          await med('REAL ' + tool);
        }
