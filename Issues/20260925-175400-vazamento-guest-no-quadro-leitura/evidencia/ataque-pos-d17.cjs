/*
  ATAQUE DO CHEFE APOS O D17 (auditoria da correcao): a trava de "somente leitura" do quadro de sessão
  ...
  Duas metades, na MESMA execucao:
   (1) o vazamento do D16 nao pode mais acontecer (quadro em leitura imune ao Guest de outra sessao);
   (2) REGRESSAO: com o quadro da sessao VIVA aberto, o traco do Guest PRECISA continuar chegando ao Host.
  Um funil rigoroso demais passa em (1) e quebra a aula ao vivo em (2) — por isso as duas medidas juntas.
  Prova: contagem de elementos renderizados + PIXELS DA TELA REAL (page.screenshot).
*/
...
async function main() {
  ...
  await page.screenshot({ path: path.join(OUT, 'pos-d17-leitura-antes.png') });
  ...
  await page.screenshot({ path: path.join(OUT, 'pos-d17-leitura-depois.png') });
  ...
  // ========== SEGUNDA METADE: regressao do caminho feliz (aula ao vivo) ==========
  await page.click('#btn-voltar-sessao');
  await page.waitForSelector('#btn-abrir-nova-sessao', { timeout: 10000 });
  await sleep(600);
  await page.click(`#btn-abrir-quadro-${sessaoVivaId}`);
  await page.waitForSelector('#canvas-quadro-branco', { timeout: 15000 });
  await sleep(1200);
  const vivaUi = await page.evaluate(() => ({
    badgeLeitura: Boolean(document.getElementById('badge-somente-leitura')),
    temLapis: Boolean(document.getElementById('tool-pencil')),
  }));
  const boxViva = await page.$eval('.upper-canvas', (el) => {
    const r = el.getBoundingClientRect();
    return { left: r.left, top: r.top, width: r.width, height: r.height };
  });
  const vivoAntes = await page.evaluate(() => Object.keys(window.__whiteboardEngine?.getLastRenderedState()?.elements || {}));
  const pxVivoAntes = await countScreenStrokePixels(page, boxViva);
  await page.screenshot({ path: path.join(OUT, 'pos-d17-viva-antes.png') });
  console.log(`[Ataque] Quadro da sessao VIVA aberto: ${JSON.stringify(vivaUi)} | elementos=${vivoAntes.length} | pixels=${pxVivoAntes}`);

  await guestDraw(0.7, 0.3);
  await sleep(1500);
  const vivoDepois = await page.evaluate(() => Object.keys(window.__whiteboardEngine?.getLastRenderedState()?.elements || {}));
  const pxVivoDepois = await countScreenStrokePixels(page, boxViva);
  await page.screenshot({ path: path.join(OUT, 'pos-d17-viva-depois.png') });
  const guestTotal = await guest.evaluate(() => Object.keys(window.__guestEngine?.getLastRenderedState()?.elements || {}).length);
  const novosVivos = vivoDepois.filter((id) => !vivoAntes.includes(id));
  const recebeu = novosVivos.length > 0 && pxVivoDepois - pxVivoAntes > 200;
  console.log(`[Ataque] CONTROLE -> elementos no Guest agora: ${guestTotal}`);
  console.log(`[Ataque] Sessao VIVA depois do traco do Guest: elementos=${vivoDepois.length} (novos: ${novosVivos.length}) | pixels=${pxVivoDepois} (delta ${pxVivoDepois - pxVivoAntes})`);
  console.log(recebeu
    ? '[RESULTADO 2] Caminho feliz OK: o Host recebeu e DESENHOU o traco do Guest na sessao viva.'
    : '[RESULTADO 2] REGRESSAO: o Host NAO recebeu o traco do Guest na sessao viva (o funil do D17 esta bloqueando demais).');

  await guestBrowser.close();
  await browser.disconnect();
  app.kill();
  await sleep(500);
  fs.writeFileSync(path.join(OUT, 'ataque-pos-d17-app.log'), appLog);
  process.exit(vazou || !recebeu ? 1 : 0);
}
