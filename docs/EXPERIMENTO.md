...
| 08 (abas/assets/mídia, plano) | 1 (`sessaoId` sem validar `isValidId` antes de virar componente de caminho em `asset.ipc.ts`/`asset.service.ts` — path traversal na escrita de assets) | 51 PASS / 0 FAIL (M1..M10, com pixel real em M4/M5/M9.3) | 0 (a autoauditoria não afirmou nada sobre esse ponto — foi omissão, não falso PASS) | baixo: `auditar.cjs` + leitura de trechos (token de mídia, rota `/midia`, funil tipado, sanitização) | o próprio projeto já tinha o padrão certo (`isValidId` no `abaId` do Guest) e não foi replicado no `sessaoId` do Host; corrigido pelo chefe (2 arquivos, < 15 linhas) + teste de ataque novo |
| 09 (relatório PDF) | 1 (`handleRelatorioAbrir` abria qualquer `.pdf` existente via `shell.openPath` sem restringir à raiz de arquivamento — mesma classe do achado da fase 08, `sessaoId`/caminho de disco) | 25 PASS / 0 FAIL (R1..R9, com pixel real via `paintImageXObject` extraído do PDF) | 0 (omissão disclosed, não falso PASS: a autoauditoria já admitia "sessão com 100 abas" não testada) | baixo: `auditar.cjs` + leitura de trechos (janela offscreen, interceptação de rede, escrita atômica) | 1ª rodada da AGY nesta fase entregou tudo sem interrupção, em commits pequenos; achado corrigido pelo chefe (1 arquivo, < 15 linhas) + 2 testes de ataque; duplicata de autoauditoria (`autoauditoria-09.md` vs `-relatorio-pdf.md`) removida pelo chefe |
