...
4. CI: workflow (ex.: `.github/workflows` ou equivalente compatível com o fluxo CodeCommit — decidir e documentar em ADR) que roda typecheck, testes e build. **Tem que ser executado de verdade** (dispatch/execução local equivalente com `act` ou script) e o log real colado no HANDOFF. Sem execução, escreva "não executado" — não "pronto".
